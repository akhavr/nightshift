...
`docker-entrypoint.sh` rewrites the worktree `.git` pointer to use container paths (`/repo-git/worktrees/agent-<short-id>`), enabling git operations inside the container. It also generates `~/.codex/config.toml` for Codex agent sessions: if `CODEX_BASE_URL` is set, it generates a config.toml with a custom provider; if not, it exports `OPENAI_API_KEY` from `CODEX_API_KEY` so Codex uses OpenAI natively. Env var fallback chain: `CODEX_API_KEY` → `OPENAI_API_KEY` → fail. `AGENT_KIND` is passed from `host/docker_cmd.py` based on the workflow's `agent.kind` setting.
