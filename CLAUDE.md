...
- **Auth-failure detection**: `HeadlessAgentBase._is_auth_failure()` (classmethod in `adapters/agents/base.py`) checks text against the subclass's `AUTH_FAILURE_PATTERNS` tuple. `ClaudeCodeAgent` detects Claude-specific patterns (invalid key, expired token, unauthorized, etc.) in `result`, `error`, and `system` events. `OpenHandsAgent` detects litellm/LLM API patterns (HTTP 401/429/404, litellm errors, connection errors, rate limits) in `ObservationEvent` with `is_error=true`. Both emit `AgentEventType.AUTH_FAILURE`. `SessionRunner` handles this by setting `suspended:auth-failure` status, committing WIP, and notifying the user. The watcher's `SessionMonitor.check_auth_failures()` retries these sessions on a slow interval (`AUTH_RETRY_INTERVAL_S`, 5 min) up to `MAX_AUTH_RETRIES` (6 attempts). After the limit, the session transitions to `suspended:auth-failure-permanent` and requires manual `nightshift resume`.
...
Container naming: coder containers are `nightshift-<short-id>`, review containers are `nightshift-review-<short-id>`. The watcher detects review sessions by the `review-` prefix in session IDs and passes `--step review --workflow REVIEW.md` when auto-resuming them. When `agent.kind` is `openhands`, the container runs OpenHands instead of Claude Code; `docker_cmd.py` injects the `LLM_*` env vars and any agent-specific extra_args. When `agent.kind` is `codex`, the Codex CLI is available in the image; `docker-entrypoint.sh` generates `~/.codex/config.toml` from overflow env vars (`OVERFLOW_API_KEY`, `OVERFLOW_BASE_URL`, `OVERFLOW_MODEL`, `CODEX_MODEL_PROVIDER`) when present. The `CodexAgent` adapter uses `--dangerously-bypass-approvals-and-sandbox` inside Docker (detected via `/.dockerenv`) since the container is the sandbox, and `--full-auto` on the host.
...
Python 3.12+. Runtime: `requests`, `pyyaml`, `jinja2`. The Docker image also installs `git-bug` v0.10.1, `@anthropic-ai/claude-code` (npm), and `@openai/codex` (npm).
