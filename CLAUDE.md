...
- `watchdog/` — Global watchdog for monitoring multiple watcher instances. New pipeline: `scanner.py` discovers `~/.nightshift/projects.d/*.yaml` registrations and tails watcher logs, `rules.py` detects stale logs / repeated errors / error thresholds, `llm.py` optionally runs Ollama or OpenRouter analysis, `notify.py` sends Telegram/webhook/file alerts, `config.py` loads `~/.nightshift/watchdog.yaml`, and `main.py` ties it together with `--list`, `--check`, and daemon mode. Run via `nightshift watchdog`.
