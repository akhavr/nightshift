...
`docker-entrypoint.sh` rewrites the worktree `.git` pointer to use container paths (`/repo-git/worktrees/agent-<short-id>`), enabling git operations inside the container. It also generates `~/.codex/config.toml` for Codex agent sessions: in overflow mode (`OVERFLOW_API_KEY` set), it configures the openrouter provider; in non-overflow mode (`AGENT_KIND=codex`), it uses `CODEX_API_KEY` or falls back to `ANTHROPIC_API_KEY` with the anthropic provider. `AGENT_KIND` is passed from `host/docker_cmd.py` based on the workflow's `agent.kind` setting.
