...
- **Signal method configuration**: The `signal_method` config in WORKFLOW.md's `agent` section controls which signal detection mechanisms `SessionRunner` checks: `auto` (default, checks all methods), `mcp` (MCP tool calls only), `text` (text markers like `@@DONE@@` only), `file` (polls `/session/signal/` directory only). This allows per-agent optimization — e.g., `signal_method: mcp` for Claude Code (which reliably uses MCP tools), `signal_method: file` for OpenHands (which cannot use MCP). Configured in `AgentConfig.signal_method`, validated by `loader._parse_agent_config()` against allowed values `{"auto", "mcp", "text", "file"}`.
...
- **Pre-review rebase**: When the agent outputs `@@DONE@@`, the container transitions to `waiting:review`. The host-side `review_orchestrator.maybe_launch_review()` then calls `host/rebase.py:attempt_pre_review_rebase()` **before** launching the review session. Rebase runs on the host (not in the container) to avoid bind-mount issues where git cannot unlink mounted files like WORKFLOW.md. If rebase conflicts, a merge fallback is attempted. If merge succeeds, tests run (if `test_command` is configured) and the review session is launched. Only if both rebase and merge fail is the coder session resumed with a conflict resolution prompt. On test failure after successful rebase/merge, the coder session is also resumed with a descriptive prompt.
...
Tests use mock implementations from `tests/conftest.py` (`MockAgent`, `MockTracker`, `MockNotifier`, `MockWorkspaceManager`). ~1500 tests across `tests/` and `tests/watcher/`.
...
`docker-entrypoint.sh` sets `GIT_DIR` and `GIT_WORK_TREE` environment variables to point to the mounted `/repo-git/worktrees/agent-<short-id>` and `/workspace`, enabling git operations inside the container without modifying the `.git` file. It also generates `~/.codex/config.toml` for Codex agent sessions: if `CODEX_BASE_URL` is set, it generates a config.toml with a custom provider; if not, it exports `OPENAI_API_KEY` from `CODEX_API_KEY` so Codex uses OpenAI natively. Env var fallback chain: `CODEX_API_KEY` → `OPENAI_API_KEY` → fail. `AGENT_KIND` is passed from `host/docker_cmd.py` based on the workflow's `agent.kind` setting.
