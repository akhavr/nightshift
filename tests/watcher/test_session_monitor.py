...
class TestStaleReviewSessionCleanup:
    ...
    def test_startup_cleanup_processes_verdict_first(self, tmp_path):
        """Stale review cleanup on startup processes verdict before cleanup.

        Bug scenario without fix:
        1. Watcher restarts
        2. cleanup_stale_review_sessions() cleans up review without processing verdict
        3. Coder session still in waiting:review
        4. check_for_auto_review() launches new review
        5. Infinite loop

        Fix: Process verdict BEFORE cleanup so coder state is updated.
        """
        w = _make_watcher(tmp_path)
        coder_sd = _make_session(w.sessions_dir, "abc", status="waiting:review",
                                 issue_id="issue-abc")
        ...
        # Set completed_at and add verdict to conversation log
        state = json.loads((review_sd / "state.json").read_text())
        state["completed_at"] = "2026-04-21T16:15:19.552265+00:00"
        (review_sd / "state.json").write_text(json.dumps(state))
        ...
        # Coder should be transitioned to waiting:human-review (approve verdict processed)
        coder_state = json.loads((coder_sd / "state.json").read_text())
        assert coder_state["status"] == "waiting:human-review", (
            f"Expected 'waiting:human-review' after approve verdict, got '{coder_state['status']}'"
        )

    def test_startup_cleanup_processes_revise_verdict(self, tmp_path):
        """Stale review cleanup on startup handles revise verdict - resumes coder."""
        w = _make_watcher(tmp_path)
        coder_sd = _make_session(w.sessions_dir, "abc", status="waiting:review",
                                 issue_id="issue-abc")
        review_sd = _make_session(w.sessions_dir, "review-abc", status="waiting:review",
        ...
        # Set completed_at and add revise verdict
        state = json.loads((review_sd / "state.json").read_text())
        state["completed_at"] = "2026-04-21T16:15:19.552265+00:00"
        (review_sd / "state.json").write_text(json.dumps(state))
        (review_sd / "conversation.jsonl").write_text(
        ...
        assert coder_state["status"] == "working", (
            f"Expected 'working' after revise verdict, got '{coder_state['status']}'"
        )
